...
export default function OrderAdmin() {
  ...
  useEffect(() => {
    // Cargar pedidos con sus estados guardados
    ...
    // Si ya existen pedidos guardados con sus estados, usarlos directamente
    if (pedidosAdmin.length > 0) {
      setOrders(pedidosAdmin);
      return;
    }
    
    // Si no hay pedidos guardados con estados, inicializar desde pedidos simulados
    const parsed: Order[] = pedidosSimulados.map((pedido: any, index: number) => {
      return {
        id: pedido.id || Date.now() + index + Math.floor(Math.random() * 1000),
    ...
    setOrders(parsed);
    // Guardar los pedidos iniciales en localStorage
    localStorage.setItem("pedidosAdmin", JSON.stringify(parsed));
  }, []);
